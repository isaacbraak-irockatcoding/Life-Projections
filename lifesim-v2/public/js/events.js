...
function updateMortgagePreview() {
  ...
  const annualCosts = homeVal * 0.03;
  if (homeVal > 0) {
    preview.textContent = `Mortgage: ${fmtM(principal)} at ${rate}% → ${fmtM(Math.round(monthly))}/mo · Annual costs (ins/tax/maint): ${fmtM(Math.round(annualCosts))}/yr`;
  } else {
    preview.textContent = '';
  }
}
...
async function addEvent() {
  ...
  try {
    ...
    ['ev-name','ev-emoji','ev-age','ev-cost','ev-annual',
     'ev-home-value','ev-home-appreciation','ev-mortgage-rate','ev-mortgage-years'].forEach(
  ...
  el.innerHTML = [...scenario.events].sort((a, b) => a.at_age - b.at_age).map(ev => {
    ...
    if (ev.event_type === 'house_purchase') {
      if (ev.home_value > 0) {
        ...
        parts.push(`${fmtM(Math.round(ev.home_value * 0.03))}/yr costs`);
      }
    } else {
